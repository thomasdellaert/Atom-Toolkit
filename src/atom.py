...
class EnergyLevel(BaseLevel):
    ...
    def from_dataframe(cls, df, i=0):
        ...
        and "Lande"
        TODO: Make this method more robust. have error handling, default values, etc.
...
class Atom:
    ...
    differences between levels are consistent with transition frequencies

    The Atom's internal graphs are wrapped in a dict-like interface, in the form of the attributes:
        levels, hflevels, zlevels
        transitions, hftransitions, ztransitions

    The Atom contains parameters that are global to all levels, such as the magnetic field 'B' and the
    ...
    transitions, such as branching ratios and state lifetimes #TODO: state lifetimes
    ...
    def generate_hf_csv(self, filename=None, blank=False, def_A=Q_(0.0, 'gigahertz')):
        """
        Output the heyperfine coefficients of every energy level in the atom
